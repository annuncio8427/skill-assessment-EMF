...
Conclusion:
                           The humble coaxial cable is not just a "shielded wire." It is a precision-engineered device that exploits all four Electric Boundary Conditions to perform two separate jobs simultaneously:
Rules 1 & 2 (Conductor): Create a perfect "prison" that traps the signal on the inside.
Rules 1 & 2 (Conductor): Create a perfect "shield" that terminates noise on the outside.
Rules 3 & 4 (Dielectric): Define the material properties that create a reflection-free highway for the signal.
